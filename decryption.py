...
class Decryption(object):
    ...
    # Inverse mix columns transformation on stateMatrix matrix
    def inverseMixColumns(self, stateMatrix):
        invMixCloumns = [
            self.galoisFieldMultiplication(
                9, stateMatrix[0]) ^ self.galoisFieldMultiplication(2, stateMatrix[2]),
            self.galoisFieldMultiplication(
                9, stateMatrix[1]) ^ self.galoisFieldMultiplication(2, stateMatrix[3]),
            self.galoisFieldMultiplication(
                9, stateMatrix[2]) ^ self.galoisFieldMultiplication(2, stateMatrix[0]),
            self.galoisFieldMultiplication(
                9, stateMatrix[3]) ^ self.galoisFieldMultiplication(2, stateMatrix[1]),
        ]
    ...
    def decrypt(self, ciphertext):
        ...
        # print("Pre Round Transfromation")
        # print("\nRound key k2")
        # print(self.round2_key)

        stateMatrix = self.addRoundKey(
            self.round2_key, self.intToState(ciphertext))

        stateMatrix = self.substituteNibbles(
            self.sBoxInv, self.shiftRows(stateMatrix))

        # print("\n\nRound 1")
        # print("\nRound key k1")
        # print(self.round1_key)

        stateMatrix = self.inverseMixColumns(
            self.addRoundKey(self.round1_key, stateMatrix))
        ...
        stateMatrix = self.addRoundKey(
            self.pre_round_transformation_key, stateMatrix)
